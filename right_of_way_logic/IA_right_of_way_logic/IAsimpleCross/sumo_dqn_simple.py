...
import right_of_way_logic.IA_right_of_way_logic.IAsimpleCross.crossSimple as env
import time
import csv
from Tools import simulation_parametre_tools as simu_tools, statistique as statistique


def deep_learning(display, simulation_time, reward_type, coef, flow1, flow2, flow3, flow4):
    ...
    def create_q_model():
        ...
        layer1 = layers.Conv2D(32, 3, strides=4, activation="relu")(inputs)
    ...
    while True:  # Run until solved
        ...
        state = np.array(env.reset(display))
        episode_reward = 0
        nb_frame_episode = 0
        ...
        for timestep in range(1, max_steps_per_episode):
            ...
            state_next, reward, done, average_waiting_time, cumulated_waiting_time, emission_of_co2, average_speed = \
                env.step_few(action_binaire, simulation_time, reward_type, coef)
        ...
        statistique.csvFileWriter(time_stamp, episode_average_waiting_time / nb_frame_episode,
                                  ...
                                  episode_emission_co2 / nb_frame_episode, episode_reward, episode_count)
        ...
        if frame_count % 1000 < 205:
            model.save("./model_simple/save_model_simple")
            model_target.save("./model_simple/save_model_target_simple")

        if running_reward > 315:  # Condition to consider the task solved
            print("Solved at episode {}!".format(episode_count))
            model.save("./model_simple/save_model")
            model_target.save("./model_simple/save_model_target")
